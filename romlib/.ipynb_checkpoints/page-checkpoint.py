...
class RomwodPage(object):
    ...
    def __init__(self, url, needs_login=False):
        ...
        self._parser = BeautifulSoup(self._content, 'lxml')
    ...
    def extract_video_blocks(self):
        ...
        the romwod site.
        """
        #parsed_html = BeautifulSoup(self._content, 'lxml')
        #video_dict = self._parser.find('div', {'data-react-class':'WeeklySchedule'})
        s = json.loads(re.search('\{.*\}', self._parser.find_all('script')[3].contents[0]).group())
